...
function main(args)
{
    ...
    GHbot.onMessage(async (msg, chat, user) => {
        ...
        // ✅ WELCOME HANDLER (CRASH FIXED)
        if(
            msg.chat.isGroup &&
            chat.welcome.state &&
            (!chat.captcha || !chat.captcha.state) &&
            msg.hasOwnProperty("new_chat_members")
        ){
    ...
    GHbot.onCallback((cb, chat, user) => {
        ...
        if(cb.data.startsWith("S_WELCOME_")){
            ...
            GHbot.editMessageText(user.id, text, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [
                            {text: l[lang].TURN_OFF_BUTTON, callback_data: "S_WELCOME_OFF:"+chat.id},
                            {text: l[lang].TURN_ON_BUTTON, callback_data: "S_WELCOME_ON:"+chat.id}
                        ],
                        [{text: l[lang].RULES_CHANGE_BUTTON, callback_data: "S_WELCOME#MSGMK:"+chat.id}],
                        [
                            {text: l[lang].ALWAYS_SEND_BUTTON, callback_data: "S_WELCOME_ALWAYS:"+chat.id},
                            {text: l[lang].FIRSTJOIN_SEND_BUTTON, callback_data: "S_WELCOME_ONCE:"+chat.id}
                        ],
        ...
        if(cb.data.startsWith("S_WELCOME#MSGMK")){
            ...
            var customMessage = MSGMK.callbackEvent(
                GHbot,
                db,
                chat.welcome.message,
                cb,
